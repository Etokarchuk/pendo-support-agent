...
codebase. Show code only when pointing at a specific decision. ~715 words / ~4:45 of
...
The general shape: roughly six reasons a guide might not show — unpublished, an empty
...
"I scoped this narrowly on purpose — one request family, deep, not ten kinds of
tickets done shallow, because breadth without a way to test correctness is
unfalsifiable. I also deliberately skipped: any write action, a real Pendo
integration since I don't have access, a vector database for nine short docs, and an
agent framework — the loop's about a hundred lines, and I wanted every decision in it
to be mine, not configuration."
...
the data was actually stale or conflicting. Eight are specifically about ambiguous or
conflicting situations, the harder behavior to get right. This runs once per case
today; the honest next step is running each N times and gating on a pass rate, because
a single run can't tell you 'wrong' from 'unlucky' — I learned that the hard way
building this."
...
swap not a rewrite; a deterministic check for the single most common cause, once trace
data tells me what that is; and calibrating the judge against human labels before
trusting it beyond a reported score.
